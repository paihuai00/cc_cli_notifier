...
test('ruliu provider sends message card body and checks errcode', async () => {
  ...
  try {
    await sendRuliu(
      { type: 'ruliu', enabled: true, webhookUrl: 'https://apiin.im.baidu.com/api/msg/groupmsgsend?access_token=token' },
      { title: 'Claude 完成', text: 'Claude:\n已完成，等待下一步' },
    );

    assert.equal(calls.length, 1);
    const body = JSON.parse(String(calls[0]?.init.body)) as {
      message: { header: { title: string; template: string }; body: Array<{ type: string; content: string }> };
    };
