...
import { getStringField, isRecord, type NotificationEvent } from '../notify/event.js';
...
function normalizeHookPayload(payload: unknown): Record<string, unknown> {
  return isRecord(payload) ? payload : {};
}

function isSupportedEventName(eventName: string | undefined): eventName is NotifierEventName {
  return eventName === 'Stop' || eventName === 'PermissionRequest';
}

function buildBaseEvent(eventName: NotifierEventName, hookPayload: Record<string, unknown>, cwd: string, sessionId: string | undefined): NotificationEvent {
...
export async function runHook(args: string[], logger: Logger): Promise<number> {
  ...
  if (eventName === 'Stop') {
    const summaryOptions: Parameters<typeof extractTranscriptSummary>[0] = { hookPayload, cwd, config: resolvedConfig.summary };
    if (sessionId) summaryOptions.sessionId = sessionId;
    const summary = extractTranscriptSummary(summaryOptions);
    if (summary.transcriptPath) event.transcriptPath = summary.transcriptPath;
    if (summary.lastUserMessage) event.lastUserMessage = summary.lastUserMessage;
    if (summary.lastAssistantSummary) event.lastAssistantSummary = summary.lastAssistantSummary;
  } else {
    ...
    if (permission.permissionTarget) event.permissionTarget = permission.permissionTarget;
    if (permission.permissionContent) event.permissionContent = permission.permissionContent;
  }
