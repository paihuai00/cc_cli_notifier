...
function parseOptions(args) {
    const options = { yes: args.includes('--yes') || args.includes('-y') };
...
function defaultEnvFor(provider) {
    if (provider === 'feishu')
        return 'FEISHU_BOT_WEBHOOK';
    if (provider === 'wecom')
        return 'WECOM_BOT_WEBHOOK';
    if (provider === 'ruliu')
        return 'RULIU_BOT_WEBHOOK';
...
async function promptForOptions(options) {
    ...
    try {
        if (!options.provider) {
            ...
            if (isProviderType(answer))
                options.provider = answer;
        }
        if (options.provider && !options.env) {
...
export async function runInit(args, logger) {
    ...
    if (provider)
        logger.info(`Configured ${provider.type} provider using env:${provider.envName}`);
    else
        logger.info('No provider configured yet; edit config or rerun init with --provider/--env');
    if (result.changed) {
        logger.info('Installed Claude Code Stop and PermissionRequest hooks');
        if (result.backupPath)
            logger.info(`Backed up settings: ${result.backupPath}`);
    }
    else {
